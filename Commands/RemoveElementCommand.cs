...
class RemoveElementCommand : IEditorCommand
{
	readonly FormDef _form;
	readonly ISkinElement _element;

	public string Description => $"削除: {_element.ElementKey}";

	public RemoveElementCommand(FormDef form, ISkinElement element)
	{
		_form = form;
		_element = element;
	}

	public void Execute() => RemoveFromForm(_form, _element);
	public void Undo() => AddToForm(_form, _element);
